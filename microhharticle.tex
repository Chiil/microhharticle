...
The usage of a seven-point stencil requires special care at the walls. In Fig. \ref{fig:stencil_wall} we show an example of how the second derivative in the vertical direction is computed for a scalar at the first model level (green circle in Fig. \ref{fig:stencil_wall}). The calculation of the divergence (Fig. \ref{fig:stencil_wall}, red stencil) requires the gradient located at the first face below the surface (lowest red circle in Fig. \ref{fig:stencil_wall}), which can only be acquired using the biased gradient operator (Eq. \ref{eq:grad_biased}).
...
\begin{figure}[t]
	...
	\end{center}
	\caption{Schematic of the diffusion discretization near the wall. Green is the evaluation point at the center of the first cell above the wall, red is the stencil of the divergence operator, and yellow shows the stencils of the four gradient operators over which the divergence is evaluated. White indicates the extent of the stencil.} \label{fig:stencil_wall}
\end{figure}
% \begin{eqnarray}
% \nonumber
